...
import {
  LayoutDashboard, FolderGit2, GitPullRequest, Settings, LogOut, ChevronLeft,
  ChevronRight, Shield
} from "lucide-react";
...
const navItems = [
  ...
  { path: "/prs", icon: GitPullRequest, label: "Pull Requests" },
          ...
          return (
            <button key={item.path}
              ...
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${
                active ? "bg-zinc-800/60 text-zinc-100" : "text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800/30"
              }`}
              title={collapsed ? item.label : undefined}>
              <item.icon className={`w-4 h-4 shrink-0 ${active ? "text-purple-400" : ""}`} />
